...
| **Even-split preview matches save**          | Create form mints the expense id, so the leftover cent shown before save is the one stored. A failed create mints a new id so retry does not upsert over the first row. | Upstream [#647](https://github.com/spliit-app/spliit/pull/647)                                                                                                        |
